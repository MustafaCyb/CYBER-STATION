...
async function main() {
  console.log('\n--- Create New Presentation Scaffolding for CYBER STATION ---\n');

  const titleEn = await ask('Presentation Title (EN)');
  const titleAr = await ask('Presentation Title (AR)');
  ...
  const repo = await ask('Source Repo URL', 'https://github.com/MustafaCyb/Cyber_Station-Slides');
  const folder = await ask('Source Folder URL');
  
  const session = await ask('Session Number');
  const order = await ask('Order (displays in this position)', session);

  // Load templates
  const templatePathEn = path.join(__dirname, 'templates', 'presentation-en.md');
  const templatePathAr = path.join(__dirname, 'templates', 'presentation-ar.md');
  ...
  // Write content
  const targetDirEn = path.join(__dirname, '..', 'src', 'content', 'presentations', 'en');
  const targetDirAr = path.join(__dirname, '..', 'src', 'content', 'presentations', 'ar');
  const imageDir = path.join(__dirname, '..', 'public', 'images', 'presentations', slug);
  ...
  console.log(`\n\x1b[32mSuccess!\x1b[0m Bilingual presentation created:`);
  ...
  console.log(`2. Write slide details or notes in the markdown files.`);
